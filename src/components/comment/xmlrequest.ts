 ...
 * @param {Int32} timeout 
 */
export function xhr_get(uri, onprogress_cb, onload_cb, onerror_cb, timeout = 10000){
  let req = {
    method: "GET",
    uri: uri,
    dataType: "text",
  ...
  xhr.onload = function(res){
    if(onload_cb)
      onload_cb(res.target.responseText)
    console.log("success");
  }
 ...
 * @param {Int32} timeout 
 */
export function xhr_post(uri, data, onprogress_cb, onload_cb, onerror_cb, timeout = 10000){
  let req = {
    method: "POST",
    uri: uri,
    dataType: "text",
